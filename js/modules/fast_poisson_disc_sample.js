...
export default class FastPoissonDiskSample extends CreativeTools {
  constructor(_obj) {
    super();
  ...
  move() {
    if (this.state.activeList.length > 0) {
      ...
      for (let n = 0; n < this.state.numberOfSamples; n++) {
        ...
        let magnitude = random(
          this.state.minimumDistance,
          2 * this.state.minimumDistance
        );
        ...
        if (
          newSample.x >= 0 &&
          newSample.x < this.state.width &&
          newSample.y >= 0 &&
          newSample.y < this.state.height
        ) {
          ...
          for (let i = -1; i <= 1; i++) {
            for (let j = -1; j <= 1; j++) {
              let neighbour = this.state.grid[
                (newSampleCellRow + j) * this.state.noOfCols +
                  (newSampleCellColumn + i)
              ];
              if (neighbour) {
          ...
          if (isSafe) {
            ...
            this.state.grid[
              newSampleCellColumn + newSampleCellRow * this.state.noOfCols
            ] = newSample;
